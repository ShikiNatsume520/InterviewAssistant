...
- [ ] 5.1 每阶段实现前用「文字陈述 + 必要时原型」与用户协商，细节对齐且用户说"批准"后才动正式代码
- [ ] 5.2 每阶段产出仅限本阶段实际需要的代码，不为本阶段不需要的后续文件/占位提前创建（硬性）
- [ ] 5.3 每阶段满足 ruff + mypy `--strict` + codespell + 测试通过才提验收
- [ ] 5.4 未验收不存档，未指引不自行 git 提交
