...
from agents.resume.prompts import build_plan_prompt, build_react_prompt
from agents.resume.state import ResumeState
from agents.resume.tools.crud import RESUME_CRUD_TOOLS
from kernel.config import RESUME_MODEL
from kernel.contracts import PlanConfirmPayload, StepConfirmPayload
...
def _init_llm() -> None:
    ...
    if _resume_llm is None:
        ...
        from agents.main.tools.rag_agent import rag_agent

        tools = RESUME_CRUD_TOOLS + [rag_agent]
        _resume_llm = get_chat_model(RESUME_MODEL, tools=tools)
...
def plan_node(state: ResumeState) -> dict[str, Any]:
    ...
    llm = get_chat_model(RESUME_MODEL)
    prompt = build_plan_prompt(resume=resume, intent=intent)
...
async def rag_agent_in_resume_node(
    ...
    from agents.main.tools.rag_agent import rag_agent_node
...
def step_confirm_node(state: ResumeState) -> dict[str, Any]:
    ...
    value = interrupt(StepConfirmPayload(before=before, after=after).model_dump())
