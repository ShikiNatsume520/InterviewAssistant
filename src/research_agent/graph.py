...
def _init_llms() -> None:
    ...
    if _outline_llm is None:
        _outline_llm = get_chat_model("deepseek-v4-flash")
    if _distill_llm is None:
        _distill_llm = get_chat_model("deepseek-v4-flash")
    if _finalize_llm is None:
        _finalize_llm = get_chat_model("deepseek-v4-flash")


# --------------------------------------------------------------------------- #
# outline 阶段
# --------------------------------------------------------------------------- #
OUTLINE_PROMPT = """你是深研检索规划师。给定知识缺口主题，生成 3-5 个**互补的**检索词，
...
def outline_node(state: ResearchState) -> dict[str, Any]:
    ...
    resp = _outline_llm.invoke(
        OUTLINE_PROMPT.format(gap_topic=gap_topic, feedback_block=feedback_block)
    )
...
# --------------------------------------------------------------------------- #
DISTILL_PROMPT = """你是资料提炼师。从下面这篇网页正文里, 提炼与检索主题相关的**事实性知识**,
...
def _distill(query: str, title: str, content: str) -> str:
    ...
    prompt = DISTILL_PROMPT.format(query=query, title=title, content=content[:6000])
...
FINALIZE_PROMPT = """你是知识整理师。把多轮深研搜集的笔记整合成一篇结构化 Markdown 文档,
...
def finalize_node(state: ResearchState) -> dict[str, Any]:
    ...
    if not notes:
        ...
        resp = _finalize_llm.invoke(
            FINALIZE_PROMPT.format(gap_topic=gap_topic, notes=_render_notes(notes))
        )
